...
> **⚠️ 다음 단계로 반드시 갈 것 (잊지 말 것): 덱 전체 통일.**
> v1은 활성 슬라이드 1장이지만, 이 기능의 진짜 가치는 누더기 덱(에셋마다 다른 색)을
> 하나의 컨셉으로 **덱 전체 통일**하는 데 있다. v1 검증 후 바로 이걸 다음 작업으로 올린다.
> 앵커 = 표지(또는 사용자 지정) 슬라이드, 적용 = 모든 슬라이드 일괄.

- 사용자 커스텀 팔레트 저장/편집.
- 슬라이드 전체(덱) 일괄 적용 — v1은 활성 슬라이드 1장. **(다음 단계 — 위 경고 참조)**
